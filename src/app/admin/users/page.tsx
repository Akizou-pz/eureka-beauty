  ...
  const handleTestEmail = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        alert('⚠️ Clé NEXT_PUBLIC_RESEND_API_KEY non configurée sur Vercel. Veuillez ajouter la variable NEXT_PUBLIC_RESEND_API_KEY (ou RESEND_API_KEY) dans Vercel > Settings > Environment Variables puis relancer le déploiement.');
      } else if (result?.resendResponse) {
        alert(`❌ Réponse API Resend :\n${JSON.stringify(result.resendResponse, null, 2)}`);
      } else {
        alert(`❌ Résultat d'envoi d'e-mail : ${result?.error || result?.message || 'Veuillez vérifier la clé Resend'}`);
      }
    } catch (e: any) {
