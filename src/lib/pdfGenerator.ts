...
export const generateInvoicePDF = (order: Order, formatPrice: (amount: number) => string) => {
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(...goldColor);
  doc.text('Facture d\'Achat Officielle', 196, 25, { align: 'right' });

  // Metadata block
  doc.setFontSize(8.5);
  doc.setTextColor(...grayColor);
  doc.text(`N° Facture : `, 145, 31);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...darkColor);
  doc.text(order.order_number, 196, 31, { align: 'right' });
  ...
  if (order.payment_method) {
    const pMethodText = order.payment_method === 'COD' 
      ...
      ? 'Par WhatsApp' 
      : order.payment_method;
    doc.text(`Paiement : ${pMethodText}`, 18, boxTop + 30);
  }
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...darkColor);
  doc.text('ADRESSE DE FACTURATION & LIVRAISON :', 112, boxTop + 6);
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...grayColor);
  doc.text('Statut : PAYÉ & LIVRÉ', 112, boxTop + 23);
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(...darkColor);
  doc.text('Montant Net Payé :', 130, currentY);
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(...grayColor);
  doc.text('Nous vous remercions de votre confiance. Pour tout renseignement, écrivez-nous à eurekasupplytg@gmail.com.', 105, pageHeight - 12, { align: 'center' });
